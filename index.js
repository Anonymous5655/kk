...
jQuery(async () => {
    ...
    el('connect').addEventListener('click', async () => {
        el('connect').disabled = true; el('disconnect').disabled = true; show('사이트 로그인 확인 중…');
        const password = el('password').value; el('password').value = '';
        clearAccount('계정 연결 중…');
        try {
            await api('/connect', {username: el('username').value, password});
        ...
        finally {el('connect').disabled = false; el('disconnect').disabled = false;}
    });
    ...
    el('disconnect').addEventListener('click', async () => {
        el('connect').disabled = true; el('disconnect').disabled = true;
    ...
    try {
        connected = (await (await api('/status')).json()).connected; updateGate();
        show(connected ? '계정 연결됨 · 서버 재시작 시 다시 로그인하세요.' : '계정을 연결하세요.'); await refreshAccount();
