...
export function installTransport(host, enabled, destination = '/api/plugins/sharednai-bridge/generate') {
    ...
    host.fetch = async function(input, init) {
        ...
        if (url.pathname.endsWith('/generate-direct')) return Response.json({message: 'AutoPic 생성 방식을 ST Image Generation · 기존 방식으로 선택하세요. SharedNAI 1.0은 이 모드를 지원합니다.'}, {status: 400});
        ...
        try {
            return await previous(destination, {...init, method: 'POST', body, headers, credentials: 'same-origin', signal: init?.signal ?? request?.signal});
        } catch {
